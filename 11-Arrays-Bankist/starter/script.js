...
// destruct object and arrays !!!! can make with chaining
...
// Flatmap  only goes one level nesting
...
// sort method mutates original array // string
...
console.log(++a);
console.log(a++);
console.log(a); // now increments not in focused position !
...
const convertTitleCase = function (tittle) {
  const expections = ['a', 'an', 'the', 'but', 'or', 'on', 'in', 'with'];

  const titleCase = tittle
    .toLowerCase()
    .split(' ')
    .map(word => expections.includes(word) ?
     word : word[0] + word.slice(1)).join(' ');
  return titleCase;
};
...
// final codding challenge in arrays
...
//6.
const checkEatingOkay = dog => dog.curFood > dog.recFood
* 0.9 && dog.curFood < dog.recFood * 1.1;
